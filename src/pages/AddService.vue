<script>
    ...
    import { addCloudService } from "../services/cloud-plans";
        ...
        methods: {
            async createService() {
                ...
                if(Object.keys(this.form_errors).length == 0) {
                    ...
                    await addCloudService({
                        ...this.form_data,
                    })
                    .then(() => {
                        this.notification.message = `El servicio ${this.form_data.name} ha sido creado con éxito`;
...
<template>
    <Loader v-if="addServiceLoading"/>
    <template v-else>
        <div class="flex items-center justify-center flex-col relative">
            <div class="max-w-3xl w-full px-6">
                <h1 class="text-slate-600 font-bold text-4xl pt-14 text-left">Agregar un Nuevo Servicio</h1>
                <div class="my-10 pb-10">
                    <form action="#" @submit.prevent="createService">
                        <div class="mb-5">
                            <BaseLabel for="name">Nombre</BaseLabel>
                            <BaseInput type="text" id="name" v-model="form_data.name" :disabled="addServiceLoading"/>
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <BaseInput type="text" id="price" v-model="form_data.price" :disabled="addServiceLoading"/>
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <BaseInput type="text" id="storage" v-model="form_data.storage" :disabled="addServiceLoading"/>
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <BaseInput type="text" id="users" v-model="form_data.users" :disabled="addServiceLoading"/>
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <select class="w-full border border-orange-500 rounded bg-white p-2 disabled:bg-gray-200" name="online" id="online" v-model="form_data.online" :disabled="addServiceLoading">
                                <option value="Activo">Activo</option>
                                <option value="Revisión">Revisión</option>
                                <option value="Inactivo">Inactivo</option>
                            </select>
                        </div>
                        <div class="mb-5 font-bold text-red-500" v-if="form_errors.online">
                            * {{ form_errors.online }}
                        </div>
                        <div class="pb-8 border-b-2 border-gray-300">
                            <BaseLabel for="benefits">Beneficios</BaseLabel>
                            <div class="flex items-start gap-2">
                                <textarea id="benefits" class="w-full border border-orange-500 rounded p-2 resize-none mb-0 h-[70px]" v-model="benefit" :disabled="addServiceLoading"></textarea>
                        ...
                        <div class="border-t-2 border-gray-300">
                            <MainButton :loading="addServiceLoading" size="small" class="mt-8">Finalizar Carga</MainButton>
                        </div>
                    </form>
                    <div>
